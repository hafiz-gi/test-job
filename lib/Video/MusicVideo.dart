...
class MusicStream extends StatefulWidget {
  ...
  MusicStream({this.category});

  @override
  _MusicStreamState createState() => _MusicStreamState();
}

class _MusicStreamState extends State<MusicStream> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: Provider.of<MusicProvider>(context, listen: false).fetchData(widget.category),
          ...
          if (snapshot.error != null) {
            ...
            return RotatedBox(
              ...
              child: Consumer<MusicProvider>(
                builder: (context,musicData,_){
                  return TabBarView(
                    controller: _tabController,
                    children: List.generate( musicData.musicList.length, (index) {
                      return VideoPlayerItem(
                        videoUrl: musicData.musicList[index].videoUrl,
                        size: size,
                        likes: musicData.musicList[index].likes,
                        category: musicData.musicList[index].category,
                      );
                    }),
                  );
                },
